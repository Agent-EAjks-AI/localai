...
```bash
# Enable with SQLite (default, stored at {DataPath}/auth.db)
...
| `LOCALAI_AUTH_DATABASE_URL` | `{DataPath}/auth.db` | Database URL — `postgres://...` for PostgreSQL, or a file path for SQLite |
