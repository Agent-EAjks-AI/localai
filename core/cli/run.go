...
type RunCMD struct {
	...
	AuthDatabaseURL      string `env:"LOCALAI_AUTH_DATABASE_URL,DATABASE_URL" help:"Database URL for auth (postgres:// or file path for SQLite). Defaults to {DataPath}/auth.db" group:"auth"`
...
func (r *RunCMD) Run(ctx *cliContext.Context) error {
	...
	if authEnabled {
		...
		if dbURL == "" {
			dbURL = filepath.Join(r.DataPath, "auth.db")
		}
